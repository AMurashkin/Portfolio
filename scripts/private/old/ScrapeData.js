...
// Script which scrapes http://google.com/project and generates JSON files used by this application
...
var agent = httpAgent.create('www.google.com', ['/project/']);
var baseDir = __dirname + '/../app/projects/';
var projects = [];
...
//  console.log(htmlPage);                            
  ...
  jsdom.jQueryify(window, 'http://code.jquery.com/jquery-1.4.2.min.js', function (window, jquery) {
    ...
    if (projects.length) {
      ...
      var project = {};
      project.id = agent.url.split(/\//).pop();
      project.name = body.find('h2').text().trim();
      project.description = body.find('.description').text().trim();
      project.availability = c1.find('table:nth-child(1) th:contains("Availability")+td').text().trim().split(/\s*\n\s*/),
      project.battery = {
      ...
      project.hardware = {
        ...
        audioJack: c2.find('table:nth-child(4) th:contains("Audio / headproject jack")+td').text()
      };
      project.camera= {
      ...
      project.additionalFeatures = c2.find('table:nth-child(6) td').text();
      project.images = [];
      ...
      fs.writeSync(fs.openSync(baseDir + project.id + '.json', 'w'), JSON.stringify(project));
    } else {
      var age = 0;
      body.find('ul.projectlist li.list').each(function(a){
        var url = jquery(this).find('.name a').attr('href');
        console.log('=======>', url);
        var project = {};
        project.id = url.split(/\//).pop();
        project.age = age++;
        project.imageUrl = 'http://google.com' + 
          jquery(this).find('img.project').attr('src');
        project.snippet = jquery(this).find('.description').text().trim();
        project.name = jquery(this).find('strong').text().trim();
        project.carrier = jquery(this).find('.buy-from img').attr('alt');
        project.buyUrl = jquery(this).find('.buy-from a').attr('href');
        console.log(project);
        projects.push(project);
        agent.addUrl(url);
      });
      fs.writeSync(fs.openSync(baseDir + '.json', 'w'), JSON.stringify(projects));
    }
    console.log(project);
    agent.next();
  });
