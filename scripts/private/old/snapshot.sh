...
SNAP_DIR=angular-projectcat-snapshots
...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -rf $SNAP_DIR/step-$i/app/projects
  fi

  if [[ $i = 5 ]]; then
    cp -r app/img $SNAP_DIR/step-0/app/
    cp -r app/projects $SNAP_DIR/step-0/app/
  fi
done
...
zip -r angular-projectcat-`date +"%y%m%d_%H%M"` $SNAP_DIR
